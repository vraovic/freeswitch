...
#include "aai_connector.h"
#include <stdlib.h>
#include <switch.h>

/* Prototypes */
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_aai_transcribe_shutdown);
SWITCH_MODULE_RUNTIME_FUNCTION(mod_aai_transcribe_runtime);
SWITCH_MODULE_LOAD_FUNCTION(mod_aai_transcribe_load);

SWITCH_MODULE_DEFINITION(mod_aai_transcribe, mod_aai_transcribe_load, mod_aai_transcribe_shutdown, NULL);
...
static void responseHandler(switch_core_session_t* session, const char * json) {
	...
	if (0 == strcmp("end_of_utterance", json)) {
		...
		// to create transcribe event for each session - "TRANSCRIBE_EVENT_RESULTS-uuid"
